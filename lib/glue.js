...
Glue.prototype.createFromApiBuilders = function(pattern, options, loaderName) {
  var api = poplar.create(options);
  this.loadApiBuilders(api, pattern, loaderName);
  return api;
};

Glue.prototype.loadApiBuilders = function(api, pattern, loaderName) {
  pattern = pattern || '*';

  loaderName = loaderName || 'controllers';

  var apiBuilders = (poplar.get(loaderName) || {}).__caches__ || {};
